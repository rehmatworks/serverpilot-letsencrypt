...
def main():
	...
	ap.add_argument('-w', '--wild', dest='wildcardcard', help='Obtain a wildcardcard SSL certificate. DNS challenge is required.', action='store_const', const=True, default=False)
	...
	if args.all is True:
		apps = apps()
		for app in apps:
			do_final_ssl_install(app, wildcardcard)
		...
		if(len(nonsslapps) > 0):
			...
			for nonssl in nonsslapps:
				do_final_ssl_install(nonssl, wildcard)
		else:
